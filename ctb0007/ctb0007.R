...
# ano_fonte
ctb0007_event[!is.na(data_ano), ano_fonte := "original"]
ctb0007_event[, .N, by = ano_fonte]
...
ctb0007_event[, uso_atual := NA_character_]
cobertura_cols <- c("uso_atual")
concat_columns(ctb0007_event, target = "cobertura", sources = cobertura_cols)

#vegetacao
ctb0007_event[, vegetacao := NA_character_]
ctb0007_event[, .N, by = vegetacao]

# erosao
ctb0007_event[, erosao:= NA_character_]
erosao_cols <- c("erosao")
concat_columns(ctb0007_event, target = "erosao", sources = erosao_cols)
ctb0007_event[, .N, by = erosao]
