...
data.table::setnames(ctb0021_event, old = "Situação", new = "situacao")
data.table::setnames(ctb0021_event, old = "Uso atual", new = "uso_atual")
cobertura_cols <- c("situacao", "uso_atual")
concat_columns(ctb0021_event, target = "cobertura", sources = cobertura_cols)

#vegetacao
data.table::setnames(ctb0021_event, old = "Vegetação primária", new = "vegetacao")
ctb0021_event[, vegetacao := as.character(vegetacao)]
ctb0021_event[, .N, by = vegetacao]

# erosao
data.table::setnames(ctb0021_event, old = "Erosão", new = "erosao")
erosao_cols <- c("erosao")
concat_columns(ctb0021_event, target = "erosao", sources = erosao_cols)
ctb0021_event[, .N, by = erosao]
