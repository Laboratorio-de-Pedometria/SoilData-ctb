...
data.table::setnames(ctb0070_event, old = "Situação e declive", new="situacao")
data.table::setnames(ctb0070_event, old = "Uso atual local", new="uso_atual")
cobertura_cols <- c("situacao", "uso_atual")
concat_columns(ctb0070_event, target = "cobertura", sources = cobertura_cols)

#vegetacao
data.table::setnames(ctb0070_event, old="Vegetação", new= "vegetacao")
ctb0070_event[, vegetacao := as.character(vegetacao)]
ctb0070_event[, .N, by = vegetacao]

# erosao
#  No erosion data available in the source document for this dataset.
data.table::setnames(ctb0070_event, old = "Erosão", new="erosao")
erosao_cols <- c("erosao")
concat_columns(ctb0070_event, target = "erosao", sources = erosao_cols)
ctb0070_event[, .N, by = erosao]
...
ctb0070_layer <- ctb0070_layer[order(observacao_id, profund_sup, profund_inf)]
ctb0070_layer[, camada_id := 1:.N, by = observacao_id]
ctb0070_layer[, .N, by = camada_id]
