...
data.table::setnames(ctb0011_event, old = "Situação, declive e cobertura", new = "situacao")
data.table::setnames(ctb0011_event, old = "Uso atual", new = "uso_atual")
cobertura_cols <- c("situacao", "uso_atual")
concat_columns(ctb0011_event, target = "cobertura", sources = cobertura_cols)

#vegetacao
data.table::setnames(ctb0011_event, old = "Vegetação primária", new = "vegetacao")
ctb0011_event[, vegetacao := as.character(vegetacao)]
ctb0011_event[, .N, by = vegetacao]

# erosao
data.table::setnames(ctb0011_event, old = "Erosão", new = "erosao")
erosao_cols <- c("erosao")
concat_columns(ctb0011_event, target = "erosao", sources = erosao_cols)
ctb0011_event[, .N, by = erosao]
