...
data.table::setnames(ctb0102_event, old = "Situação, Declive e Cobertura vegetal sobre o perfil", new = "situacao")
data.table::setnames(ctb0102_event, old = "Uso atual", new = "uso_atual")
cobertura_cols <- c("situacao", "uso_atual")
concat_columns(ctb0102_event, target = "cobertura", sources = cobertura_cols)

#vegetacao
data.table::setnames(ctb0102_event, old = "Vegetação Primária", new = "vegetacao")
ctb0102_event[, vegetacao := as.character(vegetacao)]
ctb0102_event[, .N, by = vegetacao]

# erosao
data.table::setnames(ctb0102_event, old = "Erosão", new = "erosao")
erosao_cols <- c("erosao")
concat_columns(ctb0102_event, target = "erosao", sources = erosao_cols)
ctb0102_event[, .N, by = erosao]
